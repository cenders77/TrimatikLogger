#ifndef TRIMATIK_HTML_H
#define TRIMATIK_HTML_H
...
<body>
<h1><img src="https://cdn0.scrvt.com/7751e89258d4a9c4a8e766355e041466/0d12219b06ae648a/4df882595f7e/viessmann_wordmark_rgb_4_white.svg" alt="Viessmann" width="160"></h1>
<div>
  Trimatik-MC:<br>
  <textarea id="msg" name="msgBox" rows="15" cols="58" style="display:block;float:left;background-color:#cccccc">-leer-</textarea>
  <div class="sliderContainer">
    <div>
      <label for="rtTag">&#9788;</label><br>
      <input type="range" min="-9" max="8" step="1" value="0" class="sliderDial" id="rtTag"><br>
      <output id="rtTagVal">N</output><br><br>
      <label for="tbGas">Gaszähler</label><br>
      <input id="tbGas" type="number" min="0" max="65535" step="0.01" size="8" value="0" class="numberBox" disabled><br>
      <button id="setGas" class="button" disabled>Setzen</button>
    </div>
    <div>
  ...
  <div id="svgscroll" style="background-color: Tan; overflow-x: scroll; overflow-y: auto; padding:5px; direction:rtl;clear:left;">
    <svg id="svg" viewBox="-10 -10 3620 230" xmlns="http://www.w3.org/2000/svg" height="230">
    <g id="heizung" style="fill:lightpink;"></g>
    <g id="labels" style="font: normal 18px sans-serif;text-anchor:middle;" fill="gray"></g>
    <g transform="scale(1,-1) translate(0.5,-190.25)" fill="none" stroke="black">
      ...
      <path id="axisxy" d="M0,-10V200 M-10,0H3600"/>
      <path id="graphSoll" stroke="magenta" d=""><title>Sollvorlauf</title></path>
      <path id="graphXxx" stroke="white"  d=""><title>Reserve</title></path>
      <path id="graphX25" stroke="yellow" opacity="0.6" d=""><title>Relais 0x25</title></path>
      <path id="graphX45" stroke="magenta" opacity="0.5" d=""><title>Schaltuhr 0x45</title></path>
      <path id="graphGas" stroke="black"  d=""><title>Leistung [1 Strich = 2,5 kW]</title></path>
      <path id="graphAussen" stroke="green"  d=""><title>Aussentemperatur</title></path>
  ...
  <p><button id="refresh" class="button">Diagramm aktualisieren</button><br>
    <a href='update'>FW-Update</a> - <a href='info.txt'>ESP-Info</a> - <a href='messwerte.csv'>Messwerte-Download (CSV)</a> - <a href='wifiscan.txt'>WLAN-Scan</a> - <a href='logreport.txt'>Log-Report</a>
    <button id="shrink" class="button">shrink</button>
    <button id="wifiN" class="button">802.11n</button>
    <button id="wifiG" class="button">802.11g</button></p>
  <p>Status: <span id='status'>undefined</span></p>
</div>
<small>build )rawliteral" __DATE__ " " __TIME__ R"rawliteral(</small>
<script>
...
function onClose(event)
{
  ...
  document.getElementById("tbGas").disabled = true;
  document.getElementById("setGas").disabled = true;
  gasLocked = true;
}
...
function onMessage(event)
{
  ...
  if(relais & 1)  // wenn der Brenner aktiv ist: keine Änderung am Gas
  {
    if(!gasLocked)
    {
      document.getElementById("tbGas").disabled = true;
    ...
    let gas = parseFloat(msg[1].substr(5));
    document.getElementById('tbGas').value = gas;
  }
  else
  {
    if(gasLocked)
    {
      document.getElementById("tbGas").disabled = false;
      document.getElementById("setGas").disabled = false;
      gasLocked = false;
    }
  }
}
function sendGasMsg(event)
{
  if(websocket.readyState == WebSocket.OPEN)
  {
    websocket.send("Gas:"+document.getElementById('tbGas').value*100);
  }
}
...
function sendWifiNMsg(event)
{
  if(websocket.readyState == WebSocket.OPEN)
  {
    websocket.send("WiFi:n");
  }
}
function sendShrinkMsg(event)
{
  if(websocket.readyState == WebSocket.OPEN)
  {
    websocket.send("LogLen:12000");
  }
}
function sendWifiGMsg(event)
{
  if(websocket.readyState == WebSocket.OPEN)
  {
    websocket.send("WiFi:g");
  }
}
  ...
  request.onload = function()
    {
      if(request.status === 200)
      {
        ...
        slider = document.getElementById('tbGas');
        slider.value = param.Gas/100;
      }
    };
...
function onLoad(event)
{
  ...
  document.getElementById('setGas').addEventListener('click', sendGasMsg);
  document.getElementById('wifiN').addEventListener('click', sendWifiNMsg);
  document.getElementById('wifiG').addEventListener('click', sendWifiGMsg);
  document.getElementById('shrink').addEventListener('click', sendShrinkMsg);
...
function updateChart()
{
  ...
  if(h*10/ymax > 3) // wenn die Werte-Striche mehr als ein Pixel auseinanderliegen
  {
    for(p=ymin; p<=ymax; p=p+10)
    {
      ...
      grf = grf + "M-3,"+y+"H"+w+" ";
      if((p<ymax) && (p%20==0)) lbl += '<text x="16" y="'+(200-y)+'">'+p+'</text>';
    }
  }
  ...
  // [5] x25 : Relais/Ausgänge
  grf = "M";
  recs = dataRelais.length;
  if(recs>0)
  {
    x = Math.round(w * (dataRelais[0][0]-tStart) / xmax);
    y = (h * (dataRelais[0][1]/3.2) / ymax).toFixed(1);
    grf = grf + " "+x+","+y;
    for(p=1; p<recs; p++)
    {
      x = Math.round(w * (dataRelais[p][0]-tStart) / xmax);
      y = (h * (dataRelais[p][1]/3.2) / ymax).toFixed(1);
      grf = grf + " H"+x+" V"+y;
    }
  }
  if(dataRelais[recs-1][0] < lastTS)
  {
    grf = grf + " H"+Math.round(w * (lastTS-tStart) / xmax);
  }
  trace = document.getElementById('graphX25');
  trace.setAttribute('d',grf);
  ...
  if(recs>0)
  {
    x = Math.round(w * (dataProgramm[0][0]-tStart) / xmax);
    y = (h * (dataProgramm[0][1]/3.2) / ymax).toFixed(1);
    grf = grf + " "+x+","+y;
    for(p=1; p<recs; p++)
    {
      x = Math.round(w * (dataProgramm[p][0]-tStart) / xmax);
      y = (h * (dataProgramm[p][1]/3.2) / ymax).toFixed(1);
      grf = grf + " H"+x+" V"+y;
    }
  }
  if(dataProgramm[recs-1][0] < lastTS)
  {
    grf = grf + " H"+Math.round(w * (lastTS-tStart) / xmax);
  }
  trace = document.getElementById('graphX45');
  trace.setAttribute('d',grf);
  
  // [8]+[11] : Status+Tag
  grf = "M";
  recs = dataStatus.length;
  if(recs>0)
  {
    x = Math.round(w * (dataStatus[0][0]-tStart) / xmax);
    y = (h * (dataStatus[0][1]/2) / ymax).toFixed(1);
    grf = grf + " "+x+","+y;
    for(p=1; p<recs; p++)
    {
      x = Math.round(w * (dataStatus[p][0]-tStart) / xmax);
      y = (h * (dataStatus[p][1]/2) / ymax).toFixed(1);
      grf = grf + " H"+x+" V"+y;
    }
  }
  if(dataStatus[recs-1][0] < lastTS)
  {
    grf = grf + " H"+Math.round(w * (lastTS-tStart) / xmax);
  }
  trace = document.getElementById('graphXxx');
  trace.setAttribute('d',grf);
...
function loadChartBIN()
{
  ...
  let skip;
  if(lastTS > 0) skip = lastTS-1; else skip = 0;
  request.open('GET', 'messwerte.bin?skip='+skip, true);
          ...
          if(len>11)
          {
            ...
            while(idx < len)
            {
              ...
              if(dt < 0)
              {
                ...
                alert(ts +':'+ dt + ' <> ' + dv.getUint16(idx-3, true)+' : ' +dv.getUint8(idx-1)+'<='+dv.getUint8(idx) + '\n' + dat);
              }
              if(dt!=0)
              {
                ts += dt;
